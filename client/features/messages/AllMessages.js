...
import {
  fetchClientMessagesAsync,
  selectClientMessages,
} from "./clientMessagesSlice";
import {
  fetchFreelancerMessagesAsync,
  selectFreelancerMessages,
} from "./freelancerMessagesSlice";
...
const AllMessages = () => {
  ...
  // If client is logged in, gets messages from state, and if none match the user, add them to new messages
  if (clientIsLoggedIn) {  
...
// if any messages have not been read and they are not from the client, add them to unread messages
    let unreadMessages = [];
    messages.map((msg) => {
...
// When page loads fetches all messaged by client
    useEffect(() => {
    ...
    return (
      <div className="messages">
        ...
        >
          ...
          {/* checks for new messages and displays them with (new message) */}
        </Typography>
        {
          newMsgs.length ? (
            newMsgs.map((message) => {
                ...
                return (
                  <>
                    <Button
                      ...
                      onClick={() => clickMessageClient(message.freelancerId)}
                    >
                      {message.freelancer.username} (new messages)
                    </Button>
                  </>
                );
              } else {
              ...
              return (
                <>
                  <Button
                    ...
                    variant="outlined"
                    startIcon={<PersonIcon />}
                    onClick={() => clickMessageFreelancer(message.clientId)}
                  >
                    {message.client.username} (new messages)
                  </Button>
                </>
              );
            } else {
