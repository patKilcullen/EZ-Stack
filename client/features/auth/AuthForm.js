...
const AuthForm = ({ name, displayName }) => {
  ...
  const handleSubmit = async (evt) => {
    ...
    if (option === "client") {
      await dispatch(
        clientAuthenticate({ username, password, method: formName })
      ).then(() => setErr(true));
    } else if (option === "freelancer") {
      await dispatch(
        freelancerAuthenticate({ username, password, method: formName })
      ).then(() => setErr(true));
    }
    // if login successful, navigte home
    if (err && !errMessage) {
      navigate("/");
    } else {
  ...
  return (
    <div className="loginPage">
      {/* if error message, display message */}
